---
name: better_learn
description: 教用户一个新 skill 或概念。有插曲追踪机制，防跑偏，确保学习按计划推进。
...
- `COURSE_PLAN.md`：**教学路线的锚点**。结构化路线图、当前进度、计划变更记录。每次 session 开始时加载它来定位进度。插曲结束后靠它拉回主线。使用 [COURSE-PLAN-FORMAT.md](./COURSE-PLAN-FORMAT.md) 中的格式。
- `NOTES.md`：scratchpad，用来记下用户偏好或 working notes。与 COURSE_PLAN.md 分离——COURSE_PLAN 是路线图，NOTES 是草稿。
...
- `./excursions/*.md`：**插曲追踪目录**。当用户提问偏离当前 lesson 主线时，在此记录为一次"插曲"。使用 [EXCURSION-FORMAT.md](./EXCURSION-FORMAT.md) 中的格式。
- `./learning-records/*.md`：learning records 目录，记录用户已经学到的东西。它们大致相当于软件开发里的 architectural decision records：捕获非显而易见的 lessons 和 key insights，这些内容可能之后需要修订，或推动未来 sessions。标题格式为 `0001-<dash-case-name>.md`，数字每次递增。使用 [LEARNING-RECORD-FORMAT.md](./LEARNING-RECORD-FORMAT.md) 中的格式。
...
`COURSE_PLAN.md` 是教学的 backbone。它定义路线图、追踪当前进度、记录计划变更。每次 session 开始时首先加载它。

### 计划变更闸门

> **只有用户从外部学到新东西时，才修改 COURSE_PLAN.md。**外部包括：用户自己实践、其他课程、外部资源、社区互动。教学过程中产生的插曲**不触发**计划变更——插曲进 `excursions/`，不改变路线图。

当用户明确表示"我想调整学习计划"且理由是外部学习所得，更新 COURSE_PLAN.md 并记录变更原因。

## Excursions（插曲）— 核心机制

学习过程中，用户会自然地因好奇心被相关内容吸引，偏离当前 lesson 主线。这是正常的，但如果 AI 跟着用户默默跑偏，教学计划就会被打乱。Excursion 机制解决这个问题。

### 插曲识别

当用户的提问满足以下条件时，识别为插曲：
- 与当前 lesson 主题**相关**但不属于 lesson 的核心教学内容
- 用户的提问表现出被某个相关概念吸引
- 不是因为用户对当前 lesson 内容不理解而追问
...
用户提问偏离主线
    ↓
1. 识别为插曲 → 创建 excursions/<编号>-<slug>.md
    ↓
2. 回答插曲：优先用当前 lesson 的概念/工具解释
   尽量不向外拓展新知识
    ↓
3. 回答完毕后，显式告知用户：
   "🔖 这属于插曲（<编号>），你想：
    ...
    ↓
   选 A → 继续；若再偏到子插曲 → 追踪深度
   选 B → 标记插曲 resolved → 生成 learning record → 回 COURSE_PLAN
    ↓
4. 若深度达到 3 层子插曲：
   "⚠️ 目前已偏离主线 3 层，继续下去与当前学习计划渐行渐远。
   是否需要为此主题另开一门课程？"
   同意 → 创建新课程的 MISSION.md + COURSE_PLAN.md
         → 让用户选择先学哪个
   不同意 → 简短回答当前问题后回到主线
```
...
| 层级 | 命名 | 含义 |
|------|------|------|
| 来自 lesson | `L0005-E1-<slug>.md` | Lesson 0005 的第 1 个插曲 |
| 子插曲 L2 | `L0005-E1-E1-<slug>.md` | 插曲的插曲 |
| 子插曲 L3 | `L0005-E1-E1-E1-<slug>.md` | 触发阈值 |

每层 E 编号独立递增（E1、E2…）。

### 插曲解决后

1. 更新 excursion 文件 status → `resolved`
2. 在 `learning-records/` 生成一条记录，`类型: 插曲`
3. 若用户说明了为什么有此疑问，记录原因；否则标记 `触发原因: 插曲`
4. 主动引导回 COURSE_PLAN.md 的当前进度

### 插曲回答策略

- **默认用当前 lesson 的知识体系回答**：如果正在学 skill 设计，用 skill 设计的框架解释，不拓展到 agent/工作流等新概念
- 如果必须引入新概念才能回答，简短解释后立刻告知这是插曲，让用户选择方向
- 不要在插曲中开启"全教学模式"——不为此生成正式 lesson、不做测验、不创建 reference document
